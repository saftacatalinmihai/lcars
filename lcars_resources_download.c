...
static bool extract_vosk_dylib_from_jar(void) {
  ...
  const char *jar_url = "https://repo1.maven.org/maven2/com/alphacephei/vosk/"
                        "0.3.45/vosk-0.3.45.jar";
  ...
  char cmd[1024];
  snprintf(cmd, sizeof(cmd), "unzip -o %s darwin/libvosk.dylib -d resources/",
           jar_path);
  int ret = system(cmd);
  if (ret != 0) {
  ...
  ret = system("mv resources/darwin/libvosk.dylib resources/libvosk.dylib && "
               "rmdir resources/darwin");
  if (ret != 0) {
  ...
  ret = system(
      "install_name_tool -id @rpath/libvosk.dylib resources/libvosk.dylib");
  if (ret != 0) {
    printf("  [WARNING] install_name_tool failed, loading might fail if rpath "
