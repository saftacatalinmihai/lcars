...
static int HandleGetEntries(int client_fd, Arena *arena) {
  ...
  rc = sqlite3_prepare_v2(
      db,
      "SELECT id, kind, title, content, value_int, value_float, done_bool, "
      "created_at_utc, last_modified_at_utc FROM entries ORDER BY id DESC;",
      -1, &stmt, NULL);
  if (rc != SQLITE_OK) {
    ...
    int resp_len =
        snprintf(resp_body, sizeof(resp_body),
                 "{\"error\":\"Failed to prepare query: %s\"}", err_msg);
    char header[512];
...
// Handles POST /entries: creates a new entry from a JSON or plain-text
// body. Returns the HTTP status code sent (200 on success, 400 on a
  ...
  if (rc == SQLITE_OK) {
    ...
    if (rc == SQLITE_DONE) {
      ...
      int resp_len = snprintf(
          ...
          "{\"status\":\"error\",\"message\":\"Execution failed: %s\"}",
          err_msg);
      char header[512];
      int header_len = snprintf(header, sizeof(header),
    ...
    int resp_len = snprintf(
        resp_body, sizeof(resp_body),
        "{\"status\":\"error\",\"message\":\"Preparation failed: %s\"}",
        err_msg);
    char header[512];
    int header_len = snprintf(header, sizeof(header),
                              "HTTP/1.1 500 Internal Server Error\r\n"
...
static void HandleHTTPConnection(int client_fd) {
  ...
  if ((strcmp(req_line.path, "/entries") == 0 ||
       strcmp(req_line.path, "/entries/") == 0) &&
      strcmp(req_line.method, "GET") == 0) {
    status_code = HandleGetEntries(client_fd, &arena);
  } else if ((strcmp(req_line.path, "/entries") == 0 ||
              strcmp(req_line.path, "/entries/") == 0) &&
             strcmp(req_line.method, "POST") == 0) {
    status_code = HandlePostEntry(client_fd, body, content_type);
  } else {
